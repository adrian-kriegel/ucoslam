...
//        if(m.second.pose_g2m.isValid())throw std::runtime_error("Map::scale with valid markers is not yet implemented");
...
void Map::applyTransform(cv::Mat m4x4){
    ...
    m44_32f_rot.at<float>(0,3)=0;
    m44_32f_rot.at<float>(1,3)=0;
    m44_32f_rot.at<float>(2,3)=0;
...
void Map::readFromFile(std::string fp){
    ...
    if (sig!=225237124)throw std::runtime_error("Map::readFromFile   is not of approrpiate type");
    fromStream(file);
}
    ...
    if(colorMarkers[0]!=-1){
        for(const auto &m:map_markers)
        {
             if (m.second.pose_g2m.isValid()){
                ...
                auto points_id=getMarkerIdPcd(m.second,colorMarkers);
