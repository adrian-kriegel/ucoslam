...
void Params::toStream(ostream &str)const{
    ...
    uint64_t start_sig=9837138769929;
    ...
    str.write((char*)&enableLoopClosure,sizeof(enableLoopClosure));
    ...
    uint64_t end_sig=1837138769922;
...
void Params::fromStream(istream &str){
    ...
    if(start_sig!=9837138769929) throw std::runtime_error("Params::fromStream Invalid signature. Different version");
    ...
    str.read((char*)&enableLoopClosure,sizeof(enableLoopClosure));
    ...
    uint64_t end_sig=0;
    while(end_sig!=1837138769922 && !str.eof())
        str.read((char*)&end_sig,sizeof(start_sig));
    if(str.eof())throw std::runtime_error("Params::fromStream Reached EOF without finding end signature");
...
void Params::saveToYMLFile(const string &path){
    ...
    fs<<"enableLoopClosure"<<enableLoopClosure;
...
void Params:: readFromYMLFile(const string &filePath){
    ...
    attemtpRead("enableLoopClosure",enableLoopClosure,fs);
