...
float GridExtractor::getMinDescDistance()const{
    ...
    case F2D_GRID_SIFT:return 90000;
    default: return 0;
    };
}
  ...
  DescriptorTypes::Type GridExtractor::getDescriptorType()const{
      ...
      case F2D_GRID_SIFT:return DescriptorTypes::DESC_SIFT;
      };
...
//    mask.copyTo(maskx);
  //  detector->detectAndCompute(imagex,maskx,*vkeypoints, *vdesc,false);
    ...
    if(_ftype==F2D_GRID_SIFT){//needs to set the octaves
        for(auto &kp:vkeypoints) {
            kp.octave=(kp.size/_featParams.scaleFactor)+0.5;
        }
